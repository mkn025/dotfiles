...
if [[ $# -eq 1 ]]; then
  ...
  selected=$(find ~/Dev/ ~/Documents/projekter/ ~/Documents/typstFiler/ ~/Dev/UiB/ ~/Dev/UiB/inf112/ ~/Dev/UiB/exPhil/ ~/Dev/UiB/inf221/semesterProjekt/ ~/Dev/UiB/inf221/ ~/skript/   ~/Dev/UiB/inf222/ ~/Dev/UiB/inf222/topics/ ~/Dev/UiB/inf222/forelesning_13_jan/ ~ -mindepth 1 -maxdepth 1 -type d | fzf --tmux center,80%,border-native )
fi
...
if [[ -z $TMUX ]] && [[ -z $tmux_running ]]; then
  tmux new-session -s $selected_name -c $selected nu
  exit 0
fi

# Create a new detached session if it doesn't exist
if ! tmux has-session -t=$selected_name 2>/dev/null; then
  tmux new-session -ds $selected_name -c $selected nu
  nu
fi
